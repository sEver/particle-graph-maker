var particleDrivenMap = function(input_data){//
  ...
  */
  ...
  var Nexus = function(){
    ...
    this.ROUND_COORDINATES = false;
    this.OPT_SPRITE_NODE = localStorage[''] || false;
    this.RENDER_METHOD = 'direct-eco';
    this.RENDER_METHOD_NUMBER = 0;
// 0 1 2 - eco(30fps), normal(60fps), and angry(unlimited fps)
// versions of direct render method (render to the canvas thats onscreen)

// 3 4 5 - eco, normal, angry versions of double_buffering method
// - rendering to offscreen canvas and then paining that onto screen
  ...
  gui.add(mcn,"ROUND_COORDINATES");
  gui.add(mcn,"OPT_SPRITE_NODE");
  var ctrlRenderMethod = gui.add(mcn,"RENDER_METHOD", [
    'direct-eco',
    'direct',
    'direct-angry',
    'double_buffering-eco',
    'double_buffering',
    'double_buffering-angry'
	]
	);

  ctrlRenderMethod.onChange(function(){
    switch(mcn.RENDER_METHOD){
      case 'direct-eco':
        mcn.RENDER_METHOD_NUMBER = 0;
        break;
      case 'direct':
        mcn.RENDER_METHOD_NUMBER = 1;
        break;
      case 'direct-angry':
        mcn.RENDER_METHOD_NUMBER = 2;
        break;

      case 'double_buffering-eco':
        mcn.RENDER_METHOD_NUMBER = 3;
        break;

      case 'double_buffering':
        mcn.RENDER_METHOD_NUMBER = 4;
        break;
      case 'double_buffering-angry':
        mcn.RENDER_METHOD_NUMBER = 5;
        break;
  ...
  var node_sprite = null;
  var node_sprite_ctx = null;
  ...
  function init(){
    ...
    var i = settings.PARTICLE_NUMBER;
    while (i--) {
    ...
    if(!node_sprite){//Drawing once the sprite we can later use
      var radius = settings.node_radius;
      // Drawing the sprite once
      node_sprite = document.createElement("canvas");
      node_sprite.height = node_sprite.width = 2*radius + 4;
      node_sprite_ctx = node_sprite.getContext('2d');
      //draw the node into this new canvas
      node_sprite_ctx.beginPath();
      node_sprite_ctx.lineWidth = 2;
      node_sprite_ctx.strokeStyle = settings.colors.node.regular;

      // -0.5 here was necessary to make the border
      // look the same as in the direct draw for some reason.
      node_sprite_ctx.arc(radius+1, radius+1, radius-0.5, 0, 2 * Math.PI, false);

      node_sprite_ctx.fillStyle = settings.colors.node.fill;
      node_sprite_ctx.stroke();
      node_sprite_ctx.fill();
    }
  }

  function attach_links(L,H){
    // L = linked particle list
    // H = hovered particle
    var i,n;
    i = n = L.length;
    while(i--){
  ...
  function draw_primary_links(ctx){
    if(p.links){
      var n = 0;
      var other_side;
      var k = n = p.links.length;
      while(k--){
  ...
  function draw_alternative_links(ctx){
    if(p.links_alt){
      var n = 0;
      var other_side;
      var k = n = p.links_alt.length;
      while(k--){
  ...
  function draw_all_the_alternative_links(ctx){
    	var i = particle_list.length;
      ctx.beginPath();
    	ctx.strokeStyle = settings.colors.link.alternative;
    	while(i--){
        p = particle_list[i];
      	draw_alternative_links(ctx);
      }
    	ctx.stroke();
  }

  function draw_all_the_primary_links(ctx){
    var i = particle_list.length;
    // Drawing all primary links on all nodes with a single path
    ctx.beginPath();
    ctx.strokeStyle = settings.colors.link.regular;//;
    while(i--){
      p = particle_list[i];
  		draw_primary_links(ctx);
    }
    // and a single stroke
    ctx.stroke();
  }

  function draw_all_the_nodes(ctx){
   var i = particle_list.length;
    // Drawing all primary links on all nodes with a single path
  ...
  function draw_node_directly(ctx, p){
    var radius = settings.node_radius;
    // DRAWING A CIRCLE
    ctx.beginPath();
  ...
  function draw_node_via_sprite(ctx, p){
    var radius = settings.node_radius;
    var x = p.x - (radius+1);
    var y = p.y - (radius+1);
    if(settings.ROUND_COORDINATES){
      x = Math.round(x);
      y = Math.round(y);
    }
    ctx.drawImage(node_sprite, x, y);
  }

  function draw_node(ctx){



    if(settings.OPT_SPRITE_NODE){
      draw_node_via_sprite(ctx,p);
    }else{
    	draw_node_directly(ctx,p);
    }
    ...
    if(settings.display_numbers){
      ctx.strokeText(p.number, p.x + 2, p.y-2);
    }

  }
  function apply_forces_mouse(){
    var d, f, t, dx, dy;
    ...
    if(f>10) f = 10;// hardcoded limit on the force number, ugly
  ...
  function hover_handling(p){
    // expects globals:
    // mx, my, hovered_particle
    var d, dx, dy;
    // distance
    d = ( dx = mx - p.x ) * dx + ( dy = my - p.y ) * dy;
  ...
  function apply_force_origin(p){
    // Force that is pulling the node towards it's home
    var odx, ody;
    var od, ot;

    // force pulling a particle to the place of origin
    var of = settings.ORIGIN_FORCE;
  ...
  function apply_movement(p){
    // Updating particle coordinates with their vectors
    // expects external variables h and w with height and width
    var px = p.x; // local variables are faster
    var py = p.y;
    var pvx = p.vx;
    var pvy = p.vy;
    var time_delta = 0;// number of milliseconds since last frame
    var new_x, new_y;



    // We do not move stuff that is outside boundaries.
    if((px<0)||(px>w)){
      return;
    }
    if((py<0)||(py>h)){
    ...
    if(last_frame_time){
      var this_frame_time = Date.now();
      time_delta = this_frame_time - last_frame_time;
      var how_many_frames_skipped = (time_delta / 17);

      // Correct the problem when being out of tab inflates this value too high
      if(how_many_frames_skipped > 5) how_many_frames_skipped = 5;

      new_x = px + pvx * how_many_frames_skipped;
      new_y = py + pvy * how_many_frames_skipped;
    }else{
      // Applying vectors
      new_x = px + pvx;
      new_y = py + pvy;
    ...
    // Boundaries checking
    // if after moving the node would be out of bounds,
    // negate the vector and recalculate new position
  ...
  function apply_friction(){
    ...
    var pvx = p.vx;
    var pvy = p.vy;
    var speed_min = settings.SPEED_MIN;
    var friction = settings.FRICTION;

    if (!friction) return;

    var speed_multiplier = 1-friction;

    if (Math.abs(pvx) >= speed_min){
      p.vx = pvx*speed_multiplier;
    }
    if (Math.abs(pvy) >= speed_min){
      p.vy = pvy*speed_multiplier;
    }
  }
  function draw_wysiwyg_links(){
    if(p.being_linked){
  ...
  function render_frame(ctx){
    ...
    var previous_p = 0;
    draw_all_the_primary_links(ctx);
    draw_all_the_alternative_links(ctx);
    draw_all_the_nodes(ctx);
    ...
    while(i--){
      ...
      if(settings.HOVER_DETECTION_ACTIVE){
        hover_handling(p);
        dragging_handling(p);
      }
  ...
  function create_canvas_copy(canvas){
    var my_product = null;
    my_product = document.createElement("canvas");
    my_product.width = canvas.width;
    my_product.height = canvas.height;
    return my_product;
  }
  ...
  var offscreen_canvas = create_canvas_copy(canvas);
  var offscreen_context = offscreen_canvas.getContext( '2d' );
  var os_ctx = offscreen_context;

  function double_buffering_render_loop(){
    // Apparently this apprroach isn't really faster
    // Few possible reason: render_frame to offscreen canvas
    // takes over twice as long as to an onscreen one
    // lack of Hardware Accelleration maybe?
    // plus - the drawImage overhead /o\
  ...
  function angry_direct_render_loop(){
    if(stats) stats.begin();

    render_frame(ctx);

    if(stats) stats.end();
    //window.setTimeout(angry_render_loop,0);
    window.setTimeout(next_frame,0);
  }

  function angry_double_buffering_render_loop(){
    if(stats) stats.begin();
  ...
  function eco_direct_render_loop(){
    if(stats) stats.begin();
  ...
  function eco_double_buffering_render_loop(){
  ...
  function next_frame(){
    if(settings.RENDER_METHOD_NUMBER == 0){
    	eco_direct_render_loop();
      return;
    }else if(settings.RENDER_METHOD_NUMBER == 1){
      direct_render_loop();
      return;
    }else if(settings.RENDER_METHOD_NUMBER == 2){
    	angry_direct_render_loop();
      return;
    }else if(settings.RENDER_METHOD_NUMBER == 3){
    	eco_double_buffering_render_loop();
      return;
    }else if(settings.RENDER_METHOD_NUMBER == 4){
      double_buffering_render_loop();
      return;
    }else if(settings.RENDER_METHOD_NUMBER == 5){
    	angry_double_buffering_render_loop();
      return;
    }
